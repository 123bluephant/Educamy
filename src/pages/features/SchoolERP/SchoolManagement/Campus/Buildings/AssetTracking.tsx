...
const initialAssets = [
  { id: 1, asset: 'Projector', location: 'Science Block', status: 'Active' },
  { id: 2, asset: 'Lab Computer', location: 'Lab 1', status: 'Under Maintenance' },
];

const AssetTracking: React.FC = () => {
  const [assets, setAssets] = useState(initialAssets);
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ asset: '', location: '', status: 'Active' });

  const handleAdd = () => {
    setAssets([...assets, { ...form, id: Date.now() }]);
    setForm({ asset: '', location: '', status: 'Active' });
    setShowModal(false);
  };
          ...
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl w-full max-w-md">
              ...
              <div className="space-y-4">
                <input className="w-full p-2 border rounded-lg" placeholder="Asset Name" value={form.asset} onChange={e => setForm(f => ({ ...f, asset: e.target.value }))} />
