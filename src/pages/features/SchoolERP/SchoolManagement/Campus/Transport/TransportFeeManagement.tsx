...
const initialFees = [
  { id: 1, student: 'John Doe', vehicle: 'Bus 1', amount: 1200, status: 'Paid' },
  { id: 2, student: 'Jane Smith', vehicle: 'Van 1', amount: 900, status: 'Unpaid' },
];

const TransportFeeManagement: React.FC = () => {
  const [fees, setFees] = useState(initialFees);
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ student: '', vehicle: '', amount: 0, status: 'Unpaid' });

  const handleAdd = () => {
    setFees([...fees, { ...form, amount: Number(form.amount), id: Date.now() }]);
    setForm({ student: '', vehicle: '', amount: 0, status: 'Unpaid' });
    setShowModal(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="max-w-4xl mx-auto mt-8 bg-white rounded-xl shadow-lg p-8">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-2xl font-bold text-gray-900">Transport Fee Management</h1>
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl w-full max-w-md">
              <h2 className="text-xl font-semibold mb-4">Add Transport Fee</h2>
              <div className="space-y-4">
                ...
                <input className="w-full p-2 border rounded-lg" placeholder="Amount" type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: Number(e.target.value) }))} />
...
export default TransportFeeManagement; 