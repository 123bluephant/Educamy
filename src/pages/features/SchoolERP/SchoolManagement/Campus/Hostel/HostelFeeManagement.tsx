...
const initialFees = [
  { id: 1, student: 'John Doe', hostel: 'A', amount: 5000, status: 'Paid' },
  { id: 2, student: 'Jane Smith', hostel: 'B', amount: 4500, status: 'Unpaid' },
];

const HostelFeeManagement: React.FC = () => {
  const [fees, setFees] = useState(initialFees);
  const [showModal, setShowModal] = useState(false);
  const [form, setForm] = useState({ student: '', hostel: '', amount: 0, status: 'Unpaid' });

  const handleAdd = () => {
    setFees([...fees, { ...form, amount: Number(form.amount), id: Date.now() }]);
    setForm({ student: '', hostel: '', amount: 0, status: 'Unpaid' });
    setShowModal(false);
  };
        ...
        {showModal && (
          <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center z-50">
            <div className="bg-white p-6 rounded-xl w-full max-w-md">
              <h2 className="text-xl font-semibold mb-4">Add Hostel Fee</h2>
